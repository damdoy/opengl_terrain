...
void qtree_test_neghbours_merge(){
   Noise_generator noise_gen;
   Quad_tree_node root(NULL, 2, 5, 0);
   root.set_as_root(noise_gen, QTREE_TEST_PID_DEBUG);
   root.break_qtree();
   ...
   root.children[0]->merge_qtree();

   if(root.children[2]->find_neighbour_north() == root.children[0]
      && root.children[2]->children[0]->find_neighbour_north() == root.children[0]
      && root.children[2]->children[1]->find_neighbour_north() == root.children[0])
   {
      printf("TEST PASSED\n");
   }
   else{
...
void qtree_test_sequential_merge(){
   Noise_generator noise_gen;
   Quad_tree_node root(NULL, 2, 5, 0);
   root.set_as_root(noise_gen, QTREE_TEST_PID_DEBUG);
   root.break_qtree();
   ...
   root.children[2]->break_qtree();
   root.children[3]->break_qtree();
   root.children[0]->children[0]->break_qtree();

   root.children[0]->merge_qtree();

   if(root.children[0]->find_neighbour_south() == root.children[2]
      && root.children[2]->find_neighbour_north() == root.children[0])
   {
      printf("TEST PASSED\n");
   }
   else{
