...
public:
   virtual void init(unsigned int vertical_segments = 0, unsigned int horizontal_segments = 0, bool revert_normals = false){
      GLuint pid = load_shaders_files("sphere_vshader.glsl", "sphere_fshader.glsl");
      if(pid == 0) exit(-1);

      init(pid, vertical_segments, horizontal_segments, revert_normals);
   }

   virtual void init(GLuint pid, unsigned int vertical_segments = 0, unsigned int horizontal_segments = 0, bool revert_normals = false){
      this->_pid = pid;
      glUseProgram(_pid);
      ...
      _revert_normals = revert_normals;
      ...
      if(revert_normals){
         for (size_t i = 0; i < _num_vertices*3; i++) {
            per_surface_normals[i] = -per_surface_normals[i];
            per_vertex_normals[i] = -per_vertex_normals[i];
         }
      }
...
protected:
   ...
   //algorithm taken from https://gamedev.stackexchange.com/questions/16585/how-do-you-programmatically-generate-a-sphere
