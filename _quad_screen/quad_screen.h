...
public:
   ...
   void init(GLuint texture, unsigned int screen_width, unsigned int screen_height){

      this->_width = screen_width;
      this->_height = screen_height;

      _pid = load_shaders_files("quad_screen_vshader.glsl", "quad_screen_fshader.glsl");
      if(_pid == 0) exit(-1);
      glUseProgram(_pid);
