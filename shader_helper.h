...
GLuint load_shaders_files(const char *vshader_filename, const char *fshader_filename){
   GLuint vshader_id = load_shader_file(vshader_filename, GL_VERTEX_SHADER);
   GLuint fshader_id = load_shader_file(fshader_filename, GL_FRAGMENT_SHADER);
   ...
   if(!success) {
      ...
      std::cout << "error at vertex shader \"" << vshader_filename << "\" compilation : " << log << std::endl;
   ...
   if(!success) {
      ...
      std::cout << "error at fragment shader \""<< fshader_filename <<"\" compilation : " << log << std::endl;
      glDeleteShader(vshader_id);
